...
# ═══════════════════════════════════════════════════════════════════════════════
# PDF Report (existing functionality)
# ═══════════════════════════════════════════════════════════════════════════════

def generate_issue_report_pdf(
    project_name: str,
    issues: list[dict[str, Any]],
    summary: dict[str, Any] | None = None,
) -> bytes:
    """Generate a formatted PDF report of all findings for a project."""
    pdf = FPDF()
    pdf.set_auto_page_break(auto=True, margin=20)
    pdf.add_page()

    pdf.set_font("Helvetica", "B", 20)
    pdf.cell(0, 15, "ConstructAI - QA/QC Review Report", new_x="LMARGIN", new_y="NEXT", align="C")
    pdf.set_font("Helvetica", "", 12)
    pdf.cell(0, 10, f"Project: {project_name}", new_x="LMARGIN", new_y="NEXT", align="C")
    pdf.ln(5)
    ...
    pdf.cell(0, 7, f"Critical: {critical}  |  Major: {major}  |  Minor: {minor}", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(3)
    ...
    if summary and summary.get("executive_summary"):
        ...
        pdf.multi_cell(0, 5, summary["executive_summary"])
        pdf.ln(2)

        confidence = summary.get("overall_confidence")
        if confidence is not None:
            pdf.set_font("Helvetica", "I", 10)
            pdf.cell(0, 6, f"Review Confidence: {round(confidence * 100)}%", new_x="LMARGIN", new_y="NEXT")
            reasoning = summary.get("confidence_reasoning", "")
            if reasoning:
                pdf.multi_cell(0, 5, f"Reasoning: {reasoning}")
            pdf.ln(2)

        risk_areas = summary.get("top_risk_areas", [])
        if risk_areas:
            pdf.set_font("Helvetica", "B", 11)
            pdf.cell(0, 7, "Top Risk Areas:", new_x="LMARGIN", new_y="NEXT")
            pdf.set_font("Helvetica", "", 10)
            for area in risk_areas:
                pdf.cell(0, 6, f"  - {area}", new_x="LMARGIN", new_y="NEXT")
            pdf.ln(3)
    ...
    for idx, issue in enumerate(issues, 1):
        ...
        pdf.cell(0, 8, f"#{idx}  [{severity}]  {issue.get('issue_type', 'N/A')}", new_x="LMARGIN", new_y="NEXT")

        pdf.set_font("Helvetica", "", 10)
        pdf.cell(0, 6, f"Drawing: {issue.get('drawing_ref', 'N/A')}  |  Location: {issue.get('location', 'N/A')}", new_x="LMARGIN", new_y="NEXT")
        ...
        if fix:
            pdf.multi_cell(0, 5, f"Suggested Fix: {fix}")

        clause = issue.get("code_clause", "")
        if clause:
            pdf.cell(0, 6, f"Code Clause: {clause}", new_x="LMARGIN", new_y="NEXT")

        pdf.cell(0, 6, f"Status: {issue.get('status', 'Open')}  |  Agent: {issue.get('agent_source', 'N/A')}", new_x="LMARGIN", new_y="NEXT")
        pdf.ln(3)
